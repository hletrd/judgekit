...
export const POST = createApiHandler({
  handler: async (_req: NextRequest, { user }) => {
    ...
    for (const title of titlesToDelete) {
      const [row] = await db.select({ id: problems.id }).from(problems).where(eq(problems.title, title));
      if (row) {
    ...
    async function ensureTag(name: string): Promise<string> {
      ...
      const id = nanoid();
      await db.insert(tags).values({ id, name, createdBy: user.id, createdAt: new Date() });
      return id;
    }

    async function addTagToProblems(tagName: string, titles: string[]) {
      ...
      for (const title of titles) {
        ...
        if (!exists) {
          await db.insert(problemTags).values({ id: nanoid(), problemId: row.id, tagId });
          log.push(`Tagged "${title}" → ${tagName}`);
        }
      }
    }

    await addTagToProblems("Week 1", [
      "[P1] 두 수의 합 구하기",
      "[P1] 두 수의 차 구하기",
      "[P1] 두 수의 곱 구하기",
    ]);

    await addTagToProblems("Week 2", [
      "[P3] 배열 뒤집기",
      "[P4] 팩토리얼 계산하기",
      "[P5] 중복 제거하기",
      "[P5] 완전수 찾기",
    ...
    const weekTags = await db
      .select({ id: tags.id, name: tags.name })
      .from(tags)
      .where(sql`LOWER(name) LIKE 'week %'`);

    for (const tag of weekTags) {
      const normalized = tag.name.replace(/^week\s*/i, () => {
        return "Week ";
      }).replace(/\s+/g, " ").trim();
      if (normalized !== tag.name) {
        // Check if normalized name already exists
        const [existing] = await db.select({ id: tags.id }).from(tags).where(eq(tags.name, normalized));
        if (existing && existing.id !== tag.id) {
          // Merge: move all problem_tags from old tag to existing, then delete old
          await db.update(problemTags)
          ...
          await db.update(tags).set({ name: normalized }).where(eq(tags.id, tag.id));
          log.push(`Renamed tag "${tag.name}" → "${normalized}"`);
        }
      }
    }

    return apiSuccess({ operations: log.length, log });
  },
});
