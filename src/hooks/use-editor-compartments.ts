...
export function useEditorCompartments() {
  ...
  const lineWrapping = useLazyRef(() => new Compartment());

  return { language, highlight, minHeight, editability, placeholderComp, contentAttributes, customTheme, lineWrapping };
}
